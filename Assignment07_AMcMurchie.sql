...
-- 2026-03-08, Alicia McMurchie, added new views with functions
--**************************************************************************--
Begin Try
	Use Master;
	If Exists(Select Name From SysDatabases Where Name = 'Assignment07DB_AMcMurchie')
	 Begin 
	  Alter Database [Assignment07DB_AMcMurchie] set Single_user With Rollback Immediate;
	  Drop Database Assignment07DB_AMcMurchie;
	 End
	Create Database Assignment07DB_AMcMurchie;
End Try
Begin Catch
	Print Error_Number();
End Catch
go
Use Assignment07DB_AMcMurchie;
...
SELECT 
	ProductName
	,FORMAT(UnitPrice, 'C', 'eng-US') AS 'Unit Price'
		FROM vProducts
...
SELECT 
	...
	,[Count] AS 'InventoryCount'
		FROM vInventories JOIN vProducts ON vInventories.ProductID = vProducts.ProductID
...
--		FROM vInventories 
--			JOIN vProducts ON vInventories.ProductID = vProducts.ProductID
--			JOIN vCategories ON vProducts.CategoryID = vCategories.CategoryID
--				GROUP BY vInventories.InventoryDate, vCategories.CategoryName
